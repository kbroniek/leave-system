...
internal class GetUserRepository(IGraphClientFactory graphClientFactory, ILogger<GetUserRepository> logger, IMemoryCache memoryCache) : IGetUserRepository
{
    ...
    private static readonly string[] Select = ["id", "displayName", "givenName", "surname", "accountEnabled", "mail", "userPrincipalName", "identities"];
    ...
    public async Task<Result<IGetUserRepository.User, Error>> GetUser(string id, CancellationToken cancellationToken)
    {
        try
        {
            ...
            return new IGetUserRepository.User(user.Id ?? id, user.DisplayName, user.GivenName, user.Surname, null, user.AccountEnabled, user.Identities?.FirstOrDefault(i => i.SignInType == "emailAddress")?.IssuerAssignedId ?? user.Mail ?? user.UserPrincipalName);
        }
