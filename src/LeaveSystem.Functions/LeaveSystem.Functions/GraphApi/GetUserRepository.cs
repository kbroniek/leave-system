...
internal class GetUserRepository(IGraphClientFactory graphClientFactory, ILogger<GetUserRepository> logger, IMemoryCache memoryCache) : IGetUserRepository
{
    ...
    private static readonly string[] Select = ["id", "displayName", "givenName", "surname", "jobTitle", "accountEnabled", "mail", "userPrincipalName", "identities"];
    ...
    public async Task<Result<IGetUserRepository.User, Error>> GetUser(string id, CancellationToken cancellationToken)
    {
        try
        {
            ...
            return CreateUserModel(user, id);
        }
    ...
    private static async Task<Result<IReadOnlyCollection<IGetUserRepository.User>, Error>> GetUsersFromGraph(IGraphClientFactory graphClientFactory, ILogger<GetUserRepository> logger, string[] ids, CancellationToken cancellationToken)
    {
        try
        {
            ...
            var users = await graphClient.Users
                .GetAsync(_ =>
                {
                    _.QueryParameters.Select = Select;
    ...
    private static IGetUserRepository.User CreateUserModel(User user, string? fallbackId = null) =>
            new(user.Id ?? fallbackId ?? throw new InvalidOperationException("User ID is required"),
                user.DisplayName, user.GivenName, user.Surname, user.JobTitle, user.AccountEnabled,
                user.Identities?.FirstOrDefault(i => i.SignInType == "emailAddress")?.IssuerAssignedId ?? user.Mail ?? user.UserPrincipalName);
}
