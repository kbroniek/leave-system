...
public static class HttpQueryExtensions
{
    ...
    public static Result<Guid[], Error> TryGuids(this IQueryCollection query, string paramName)
    {
        ...
        if (failureResults.Length > 0)
        {
            ...
            return new Error($"Cannot parse the {paramName} query parameter to Guid type.{Environment.NewLine}{serializedErrors}", System.Net.HttpStatusCode.BadRequest);
        }
    ...
    private static Result<Guid, Error> ParseGuid(string? input) =>
        ...
        result :
        new Error($"Cannot parse the {input}.", System.Net.HttpStatusCode.BadRequest);
    ...
    public static Result<int?, Error> TryInt(this IQueryCollection query, string paramName)
    {
        ...
        return int.TryParse(paramValue, out var result) ?
            result :
            new Error($"Cannot parse the {paramName} query parameter to int.", System.Net.HttpStatusCode.BadRequest);
    }
