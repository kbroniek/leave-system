...
export const AccountPicker = (props: {
  onClose: (event: AccountInfo | null) => void;
  open: boolean;
}) => {
  ...
  return (
    <Dialog
      onClose={(event: AccountInfo) => onClose(event)}
      aria-labelledby="simple-dialog-title"
      open={open}
    >
      <DialogTitle id="simple-dialog-title">
        <Trans>Set active account</Trans>
      </DialogTitle>
      <List>
        {accounts.map((account) => (
          ...
          >
            ...
            <ListItemText
              primary={
                account.name ?? (account.idTokenClaims?.signInName as string)
              }
