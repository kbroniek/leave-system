...
public class BasicValidator
{
        ...
        TimeSpan maxDuration, bool? includeFreeDays)
    {
        ...
        {
            throw new ArgumentOutOfRangeException(nameof(@event.DateFrom), "The date is off work.");
        }

        var dateToDayKind = DateCalculator.GetDayKind(@event.DateTo);
        if (dateToDayKind != DateCalculator.DayKind.WORKING)
        {
            throw new ArgumentOutOfRangeException(nameof(@event.DateTo), "The date is off work.");
        }
    }
}
